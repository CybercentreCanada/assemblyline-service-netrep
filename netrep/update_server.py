...
class NetRepUpdateServer(ServiceUpdater):
    ...
    def import_update(self, files_sha256, _, source_name, __):
        with TinyDB(self.blocklist_path) as blocklist:
            ...
            blocklist.table_class.document_id_class = str
            ...
            def update_blocklist(ioc_type, ioc_value, malware_family):
                table = blocklist.table(ioc_type)
                ...
                if ioc_type == "uri":
                    ...
                    for host_ioc_type, check in IOC_CHECK.items():
                        if check(hostname):
                            if blocklist.table(host_ioc_type).get(doc_id=hostname):
                ...
                if source_cfg["format"] == "csv":
                    ...
                    for file, _ in files_sha256:
                        with open(file, "r") as fp:
                            for row in [r.strip() for r in fp.readlines()][
                                start_index:
                            ]:
                                if '","' in row:
